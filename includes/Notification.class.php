 ...
 * notification. So, there would be a wtbNotificationEmail class or a
 * wtbNotificationSMS class.
 *
 * @since 0.0.1
 */
abstract class wtbNotification {
	...
	/**
	 * Set booking data passed from wtbNotifications
	 *
	 * @var object $booking
	 * @since 0.0.1
	 */
	...
	public function get_template( $type ) {
		...
		$template = $wtb_controller->settings->get_setting( $type );
	...
	public function process_template( $message ) {
		...
		$template_tags = array(
			...
			'{bookings_link}'	=> '<a href="' . admin_url( 'admin.php?page=wtb-bookings&status=pending' ) . '">' . __( 'View pending bookings', 'wp-table-bookings' ) . '</a>',
			'{confirm_link}'	=> '<a href="' . admin_url( 'admin.php?page=wtb-bookings&wtb-quicklink=confirm&booking=' . esc_attr( $this->booking->ID ) ) . '">' . __( 'Confirm this booking', 'wp-table-bookings' ) . '</a>',
			'{close_link}'		=> '<a href="' . admin_url( 'admin.php?page=wtb-bookings&wtb-quicklink=close&booking=' . esc_attr( $this->booking->ID ) ) . '">' . __( 'Reject this booking', 'wp-table-bookings' ) . '</a>',
		...
		$template_tags = apply_filters( 'wtb_notification_template_tags', $template_tags, $this );
