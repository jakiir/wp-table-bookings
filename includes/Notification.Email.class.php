 ...
 * @since 0.0.1
 */
class wtbNotificationEmail extends wtbNotification {
	...
	public function set_to_email() {
		...
		if ( $this->target == 'user' ) {
			...
			$to_email = $wtb_controller->settings->get_setting( 'admin-email-address' );
		}

		$this->to_email = apply_filters( 'wtb_notification_email_to_email', $to_email, $this );
	...
	public function set_from_email() {
		...
		if ( $this->target == 'user' ) {
			$from_email = $wtb_controller->settings->get_setting( 'reply-to-address' );
			$from_name = $wtb_controller->settings->get_setting( 'reply-to-name' );
		} else {
			$from_email = $this->booking->email;
			$from_name = $this->booking->name;
		}

		$this->from_email = apply_filters( 'wtb_notification_email_from_email', $from_email, $this );
		$this->from_name = apply_filters( 'wtb_notification_email_from_name', $from_name, $this );
	...
	public function set_subject() {
		...
		if( $this->event == 'new_submission' ) {
			if ( $this->target == 'admin' ) {
				$subject = $wtb_controller->settings->get_setting( 'subject-booking-admin' );
			} elseif ( $this->target == 'user' ) {
				$subject = $wtb_controller->settings->get_setting( 'subject-booking-user' );
			}

		} elseif ( $this->event == 'pending_to_confirmed' ) {
			$subject = $wtb_controller->settings->get_setting( 'subject-confirmed-user' );

		} elseif ( $this->event == 'pending_to_closed' ) {
			$subject = $wtb_controller->settings->get_setting( 'subject-rejected-user' );
		...
		$this->subject = $this->process_subject_template( apply_filters( 'wtb_notification_email_subject', $subject, $this ) );
	...
	public function set_headers( $headers = null ) {
		...
		$headers = "From: " . stripslashes_deep( html_entity_decode( $wtb_controller->settings->get_setting( 'reply-to-name' ), ENT_COMPAT, 'UTF-8' ) ) . " <" . apply_filters( 'wtb_notification_email_header_from_email', get_option( 'admin_email' ) ) . ">\r\n";
		$headers .= "Reply-To: =?utf-8?Q?" . quoted_printable_encode( $this->from_name ) . "?= <" . $this->from_email . ">\r\n";
		$headers .= "Content-Type: text/html; charset=utf-8\r\n";

		$this->headers = apply_filters( 'wtb_notification_email_headers', $headers, $this );
	...
	public function set_message() {
		...
		$template = apply_filters( 'wtb_notification_email_template', $template, $this );
	...
	public function process_subject_template( $subject ) {
		...
		$template_tags = apply_filters( 'wtb_notification_email_subject_template_tags', $template_tags, $this );
