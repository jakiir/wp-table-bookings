...
if ( !class_exists( 'wtbAdminPageSettingLicenseKey' ) ) {
/**
 ...
 * @since 1.4.1
 */
class wtbAdminPageSettingLicenseKey {
	...
	public function display_setting() {
		...
		$status_string = !empty( $this->strings[ $status] ) ? $this->strings[ $status ] : __( 'Invalid', 'wp-table-bookings' );
		...
		<div class="wtb-license-setting" data-id="<?php echo esc_attr( $this->id ); ?>">
	...
	public function sanitize_callback_wrapper( $values ) {
		...
		$old = $wtb_controller->settings->get_setting( $this->id );
	...
	public function process_action() {
		...
		if ( !current_user_can( 'manage_options' ) || empty( $_GET['tab'] ) || $_GET['tab'] !== 'wtb-licenses' || empty( $_GET['action'] ) || empty( $_GET['id'] ) || $_GET['id'] !== $this->id ) {
			return;
		}
	...
	public function process_activation_response( $license_data ) {
		...
		$wtb_settings = get_option( $this->page );
		$wtb_settings[ $this->id ] = $this->value;

		update_option( $this->page, $wtb_settings );
