 ...
 * @since 0.0.1
 */
if ( !function_exists( 'wtb_booking_form_shortcode' ) ) {
function wtb_booking_form_shortcode() {
	return wtb_print_booking_form();
}
add_shortcode( 'booking-form', 'wtb_booking_form_shortcode' );
} // endif;
 ...
 * @since 0.0.1
 */
if ( !function_exists( 'wtb_print_booking_form' ) ) {
function wtb_print_booking_form() {

	global $wtb_controller;

	// Only allow the form to be displayed once on a page
	if ( $wtb_controller->form_rendered === true ) {
		return;
	} else {
		$wtb_controller->form_rendered = true;
	}
	...
	$output = apply_filters( 'wtb_booking_form_html_pre', '' );
	if ( !empty( $output ) ) {
		return $output;
	}
	...
	if ( !empty( $_POST['action'] ) && $_POST['action'] == 'booking_request' ) {
		...
		if ( get_class( $wtb_controller->request ) === 'stdClass' ) {
			require_once( WTB_PLUGIN_DIR . '/includes/Booking.class.php' );
			$wtb_controller->request = new wtbBooking();
		}

		$wtb_controller->request->insert_booking();
	}

	// Define the form's action parameter
	$booking_page = $wtb_controller->settings->get_setting( 'booking-page' );
...
<div class="wtb-booking-form">
	<?php if ( $wtb_controller->request->request_inserted === true ) : ?>
	<div class="wtb-message">
		<p><?php echo $wtb_controller->settings->get_setting( 'success-message' ); ?></p>
	</div>
	<?php else : ?>
	<form method="POST" action="<?php echo esc_attr( $booking_page ); ?>">
		<input type="hidden" name="action" value="booking_request">

		<?php do_action( 'wtb_booking_form_before_fields' ); ?>
		...
		?>
		<fieldset <?php echo wtb_print_element_class( $fieldset, $fieldset_classes ); ?>>

			<?php if ( !empty( $contents['legend'] ) ) : ?>
			<legend <?php echo wtb_print_element_class( '', $legend_classes ); ?>>
		...
		<?php do_action( 'wtb_booking_form_after_fields' ); ?>

		<?php
			$button = sprintf(
				'<button type="submit">%s</button>',
				apply_filters( 'wtb_booking_form_submit_label', __( 'Request Booking', 'wp-table-bookings' ) )
			);

			echo apply_filters( 'wtb_booking_form_submit_button', $button );
		?>
	...
	$output = apply_filters( 'wtb_booking_form_html_post', $output );
 ...
 * @since 0.0.1
 */
if ( !function_exists( 'wtb_enqueue_assets' ) ) {
function wtb_enqueue_assets() {

	global $wp_scripts;

	wp_enqueue_style( 'wtb-booking-form' );
	...
	wp_enqueue_script( 'wtb-booking-form' );
	...
	wp_localize_script(
		...
		'wtb_pickadate',
		array(
			'date_format' => $wtb_controller->settings->get_setting( 'date-format' ),
			'time_format'  => $wtb_controller->settings->get_setting( 'time-format' ),
			'disable_dates'	=> wtb_get_datepicker_rules(),
			'schedule_open' => $wtb_controller->settings->get_setting( 'schedule-open' ),
			'schedule_closed' => $wtb_controller->settings->get_setting( 'schedule-closed' ),
			'early_bookings' => current_user_can( 'manage_bookings' ) ? '' : $wtb_controller->settings->get_setting( 'early-bookings' ),
			'late_bookings' => current_user_can( 'manage_bookings' ) ? '' : $wtb_controller->settings->get_setting( 'late-bookings' ),
			'date_onload' => $wtb_controller->settings->get_setting( 'date-onload' ),
			'time_interval' => $wtb_controller->settings->get_setting( 'time-interval' ),
		)
	);
 ...
 * @since 0.0.1
 */
if ( !function_exists( 'wtb_get_datepicker_rules' ) ) {
function wtb_get_datepicker_rules() {
	...
	return apply_filters( 'wtb_datepicker_disable_rules', $disable_rules, $schedule_open, $schedule_closed );
 ...
 *
 * @uses wtb_print_form_tick_field
 * @since 1.3.1
 */
if ( !function_exists( 'wtb_print_form_checkbox_field' ) ) {
function wtb_print_form_checkbox_field( $slug, $title, $value, $args ) {

	$slug = esc_attr( $slug );
	$value = !empty( $value ) ? array_map( 'esc_attr', $value ) : array();
	$options = is_array( $args['options'] ) ? $args['options'] : array();
	$classes = isset( $args['classes'] ) ? $args['classes'] : array();
	$classes[] = 'wtb-checkbox';
	...
	<div <?php echo wtb_print_element_class( $slug, $classes ); ?>>
		...
		<?php foreach ( $options as $opt_value => $opt_label ) : ?>
		<label>
			<input type="checkbox" name="wtb-<?php echo $slug; ?>[]" id="wtb-<?php echo $slug; ?>-<?php echo esc_attr( $opt_value ); ?>" value="<?php echo esc_attr( $opt_value ); ?>"<?php echo !empty( $value ) && in_array( $opt_value, $value ) ? ' checked' : ''; ?>>
			<?php echo $opt_label; ?>
		</label>
		<?php endforeach; ?>
	</div>
 ...
 * @since 1.3.1
 */
if ( !function_exists( 'wtb_print_form_radio_field' ) ) {
function wtb_print_form_radio_field( $slug, $title, $value, $args ) {
	...
	<div <?php echo wtb_print_element_class( $slug, $classes ); ?>>
		...
		<?php foreach ( $options as $opt_value => $opt_label ) : ?>
		<label>
			<input type="radio" name="wtb-<?php echo $slug; ?>" id="wtb-<?php echo $slug; ?>" value="<?php echo esc_attr( $opt_value ); ?>" <?php checked( $opt_value, $value ); ?>>
			<?php echo $opt_label; ?>
		</label>
		<?php endforeach; ?>
	</div>
 ...
 * @since 1.3.1
 */
if ( !function_exists( 'wtb_print_form_confirm_field' ) ) {
function wtb_print_form_confirm_field( $slug, $title, $value, $args ) {
	...
	<div <?php echo wtb_print_element_class( $slug, $classes ); ?>>
		<?php echo wtb_print_form_error( $slug ); ?>
		<label for="wtb-<?php echo $slug; ?>">
			<input type="checkbox" name="wtb-<?php echo $slug; ?>" id="wtb-<?php echo $slug; ?>" value="1" <?php checked( $value, 1 ); ?>>
			<?php echo $title; ?>
		</label>
	</div>
...
function wtb_print_form_error( $field ) {
	...
	if ( !empty( $wtb_controller->request ) && !empty( $wtb_controller->request->validation_errors ) ) {
		foreach ( $wtb_controller->request->validation_errors as $error ) {
