...
class wtbCompatibility {
	...
	public function __construct() {
		...
		define( 'WTB_TEXTDOMAIN', 'wtbdomain' );

		// Load a .mo file for an old textdomain if one exists
		add_filter( 'load_textdomain_mofile', array( $this, 'load_old_textdomain' ), 10, 2 );

		// Run a filter deprecrated in 1.4.3
		add_filter( 'wtb_bookings_table_views_date_range', array( $this, 'rtn_bookings_table_views_schedule' ) );

		// Make sure custom fields don't completely disappear in 1.5
		add_action( 'admin_init', array( $this, 'maybe_bridge_cffwtb_to_1_5' ) );
	 ...
	 *
	 * Old textdomain: wtbdomain
	 */
	public function load_old_textdomain( $mofile, $textdomain ) {

		if ( $textdomain === 'wp-table-bookings' && 0 === strpos( $mofile, WP_LANG_DIR . '/plugins/'  ) && !file_exists( $mofile ) ) {
			$mofile = dirname( $mofile ) . DIRECTORY_SEPARATOR . str_replace( $textdomain, 'wtbdomain', basename( $mofile ) );
		}
	 ...
	 * @since 0.1
	 */
	public function maybe_bridge_cffwtb_to_1_5() {
		if ( !function_exists( 'cffwtbInit' ) || !function_exists( 'get_plugin_data' ) ) {
			return;
		}

		$cffwtb = get_plugin_data( cffwtbInit::$plugin_dir . '/custom-fields-for-wtb.php', false );
		if ( !isset( $cffwtb['Version'] ) || $cffwtb['Version'] >= 1.2 ) {
			return;
		}

		add_filter( 'wtb_bookings_table_column_details', array( $this, 'add_cffwtb_fields_to_details' ), 11, 2 );
	}
	 ...
	 * @since 1.5
	 */
	public function add_cffwtb_fields_to_details( $details, $booking ) {

		if ( !isset( $booking->custom_fields ) ) {
			return $details;
		}

		$fields = cffwtbInit()->fields->get_booking_fields_display_array( $booking );
