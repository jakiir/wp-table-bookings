...
class wtbBooking {
	...
	public function load_post( $post ) {
		...
		if ( get_class( $post ) == 'WP_Post' && $post->post_type == WTB_BOOKING_POST_TYPE ) {
	...
	public function load_wp_post( $post ) {
		...
		do_action( 'wtb_booking_load_post_data', $this, $post );
	}
	...
	public function load_post_metadata() {
		...
		$meta_defaults = apply_filters( 'wtb_booking_metadata_defaults', $meta_defaults );

		if ( is_array( $meta = get_post_meta( $this->ID, 'wtb', true ) ) ) {
			$meta = array_merge( $meta_defaults, get_post_meta( $this->ID, 'wtb', true ) );
		} else {
			$meta = $meta_defaults;
		}
	...
	public function insert_booking() {
		...
		do_action( 'wtb_' . $action . '_booking', $this );
	...
	public function validate_submission() {
		...
		// Date
		$date = empty( $_POST['wtb-date'] ) ? false : stripslashes_deep( $_POST['wtb-date'] );
		if ( $date === false ) {
			$this->validation_errors[] = array(
				'field'		=> 'date',
				'error_msg'	=> 'Booking request missing date',
				'message'	=> __( 'Please enter the date you would like to book.', 'wp-table-bookings' ),
			);

		} else {
			try {
				$date = new DateTime( stripslashes_deep( $_POST['wtb-date'] ) );
			} catch ( Exception $e ) {
				$this->validation_errors[] = array(
					'field'		=> 'date',
					'error_msg'	=> $e->getMessage(),
					'message'	=> __( 'The date you entered is not valid. Please select from one of the dates in the calendar.', 'wp-table-bookings' ),
				);
			}
		}

		// Time
		$time = empty( $_POST['wtb-time'] ) ? false : stripslashes_deep( $_POST['wtb-time'] );
		if ( $time === false ) {
			$this->validation_errors[] = array(
				'field'		=> 'time',
				'error_msg'	=> 'Booking request missing time',
				'message'	=> __( 'Please enter the time you would like to book.', 'wp-table-bookings' ),
			);

		} else {
			try {
				$time = new DateTime( stripslashes_deep( $_POST['wtb-time'] ) );
			} catch ( Exception $e ) {
				$this->validation_errors[] = array(
					'field'		=> 'time',
					'error_msg'	=> $e->getMessage(),
					'message'	=> __( 'The time you entered is not valid. Please select from one of the times provided.', 'wp-table-bookings' ),
				);
			}
		}
		...
		if ( is_object( $time ) && is_object( $date ) ) {
			...
			if ( !current_user_can( 'manage_bookings' ) ) {
				...
				$early_bookings = $wtb_controller->settings->get_setting( 'early-bookings' );
				if ( !empty( $early_bookings ) ) {
					...
					if ( $request->format( 'U' ) > ( current_time( 'timestamp' ) + $early_bookings_seconds ) ) {
						$this->validation_errors[] = array(
							...
							'message'	=> sprintf( __( 'Sorry, bookings can not be made more than %s days in advance.', 'wp-table-bookings' ), $early_bookings ),
						);
					}
				}

				$late_bookings = $wtb_controller->settings->get_setting( 'late-bookings' );
				if ( empty( $late_bookings ) ) {
					if ( $request->format( 'U' ) < current_time( 'timestamp' ) ) {
						$this->validation_errors[] = array(
							'field'		=> 'time',
							'error_msg'	=> 'Booking request in the past',
							'message'	=> __( 'Sorry, bookings can not be made in the past.', 'wp-table-bookings' ),
						);
					}
					...
					if ( $request->format( 'U' ) < ( current_time( 'timestamp' ) + $late_bookings_seconds ) ) {
						if ( $late_bookings >= 1440 ) {
							$late_bookings_message = sprintf( __( 'Sorry, bookings must be made more than %s days in advance.', 'wp-table-bookings' ), $late_bookings / 1440 );
						} elseif ( $late_bookings >= 60 ) {
							$late_bookings_message = sprintf( __( 'Sorry, bookings must be made more than %s hours in advance.', 'wp-table-bookings' ), $late_bookings / 60 );
						} else {
							$late_bookings_message = sprintf( __( 'Sorry, bookings must be made more than %s minutes in advance.', 'wp-table-bookings' ), $late_bookings );
						}
						$this->validation_errors[] = array(
							'field'		=> 'time',
			...
			$exceptions = $wtb_controller->settings->get_setting( 'schedule-closed' );
			if ( empty( $this->validation_errors ) && !empty( $exceptions ) ) {
				...
				if ( $exception_is_active && !$datetime_is_valid ) {
					$this->validation_errors[] = array(
						...
						'message'	=> __( 'Sorry, no bookings are being accepted then.', 'wp-table-bookings' ),
					);
				}
			}

			// Check against weekly scheduling rules
			$rules = $wtb_controller->settings->get_setting( 'schedule-open' );
			if ( empty( $exception_is_active ) && empty( $this->validation_errors ) && !empty( $rules ) ) {
				...
				if ( !$day_is_valid ) {
					$this->validation_errors[] = array(
						...
						'message'	=> __( 'Sorry, no bookings are being accepted on that date.', 'wp-table-bookings' ),
					);
				} elseif ( !$time_is_valid ) {
					$this->validation_errors[] = array(
						'field'		=> 'time',
						'error_msg'	=> 'Booking request made at an invalid time',
						'message'	=> __( 'Sorry, no bookings are being accepted at that time.', 'wp-table-bookings' ),
					);
				}
			}
		...
		$this->request_date = empty( $_POST['wtb-date'] ) ? '' : stripslashes_deep( $_POST['wtb-date'] );
		$this->request_time = empty( $_POST['wtb-time'] ) ? '' : stripslashes_deep( $_POST['wtb-time'] );

		// Name
		$this->name = empty( $_POST['wtb-name'] ) ? '' : wp_strip_all_tags( sanitize_text_field( stripslashes_deep( $_POST['wtb-name'] ) ), true ); // @todo should I limit length?
		if ( empty( $this->name ) ) {
			$this->validation_errors[] = array(
				'field'			=> 'name',
				'post_variable'	=> $this->name,
				'message'	=> __( 'Please enter a name for this booking.', 'wp-table-bookings' ),
			);
		}

		// Party
		$this->party = empty( $_POST['wtb-party'] ) ? '' : absint( $_POST['wtb-party'] );
		if ( empty( $this->party ) ) {
			$this->validation_errors[] = array(
				'field'			=> 'party',
				'post_variable'	=> $this->party,
				'message'	=> __( 'Please let us know how many people will be in your party.', 'wp-table-bookings' ),
			);

		// Check party size
		} else {
			$party_size = $wtb_controller->settings->get_setting( 'party-size' );
			if ( !empty( $party_size ) && $party_size < $this->party ) {
				$this->validation_errors[] = array(
					'field'			=> 'party',
					'post_variable'	=> $this->party,
					'message'	=> sprintf( __( 'We only accept bookings for parties of up to %d people.', 'wp-table-bookings' ), $party_size ),
				);
			}
		}

		// Email/Phone
		$this->email = empty( $_POST['wtb-email'] ) ? '' : sanitize_text_field( stripslashes_deep( $_POST['wtb-email'] ) ); // @todo email validation? send notification back to form on bad email address.
		if ( empty( $this->email ) ) {
			$this->validation_errors[] = array(
				'field'			=> 'email',
				'post_variable'	=> $this->email,
				'message'	=> __( 'Please enter an email address so we can confirm your booking.', 'wp-table-bookings' ),
			);
		}

		// Phone/Message
		$this->phone = empty( $_POST['wtb-phone'] ) ? '' : sanitize_text_field( stripslashes_deep( $_POST['wtb-phone'] ) );
		$this->message = empty( $_POST['wtb-message'] ) ? '' : nl2br( wp_kses_post( stripslashes_deep( $_POST['wtb-message'] ) ) );

		// Post Status (define a default post status is none passed)
		if ( !empty( $_POST['wtb-post-status'] ) && array_key_exists( $_POST['wtb-post-status'], $wtb_controller->cpts->booking_statuses ) ) {
			$this->post_status = sanitize_text_field( stripslashes_deep( $_POST['wtb-post-status'] ) );
		} else {
			$this->post_status = 'pending';
		}

		// Check if any required fields are empty
		$required_fields = $wtb_controller->settings->get_required_fields();
		foreach( $required_fields as $slug => $field ) {
			if ( !$this->field_has_error( $slug ) && $this->is_field_empty( $slug ) ) {
				$this->validation_errors[] = array(
					'field'			=> $slug,
					'post_variable'	=> '',
					'message'	=> __( 'Please complete this field to request a booking.', 'wp-table-bookings' ),
				);
			}
		}

		do_action( 'wtb_validate_booking_submission', $this );
	...
	public function is_field_empty( $slug ) {
		...
		$input = isset( $_POST['wtb-' . $slug ] ) ? $_POST['wtb-' . $slug] : '';
	...
	public function insert_post_data() {
		...
		$args = array(
			'post_type'		=> WTB_BOOKING_POST_TYPE,
		...
		$args = apply_filters( 'wtb_insert_booking_data', $args, $this );
		...
		$meta = apply_filters( 'wtb_insert_booking_metadata', $meta, $this );

		return update_post_meta( $this->ID, 'wtb', $meta );
