...
if ( !class_exists( 'wtbCustomPostTypes' ) ) {
class wtbCustomPostTypes {
	...
	public function load_cpts() {
		...
		$args = array(
			'labels' => array(
				'name'               => __( 'Bookings',                   'wp-table-bookings' ),
				'singular_name'      => __( 'Booking',                    'wp-table-bookings' ),
				'menu_name'          => __( 'Bookings',                   'wp-table-bookings' ),
				'name_admin_bar'     => __( 'Bookings',                   'wp-table-bookings' ),
				'add_new'            => __( 'Add New',                 	  'wp-table-bookings' ),
				'add_new_item'       => __( 'Add New Booking',            'wp-table-bookings' ),
				'edit_item'          => __( 'Edit Booking',               'wp-table-bookings' ),
				'new_item'           => __( 'New Booking',                'wp-table-bookings' ),
				'view_item'          => __( 'View Booking',               'wp-table-bookings' ),
				'search_items'       => __( 'Search Bookings',            'wp-table-bookings' ),
				'not_found'          => __( 'No bookings found',          'wp-table-bookings' ),
				'not_found_in_trash' => __( 'No bookings found in trash', 'wp-table-bookings' ),
				'all_items'          => __( 'All Bookings',               'wp-table-bookings' ),
			),
		...
		$args = apply_filters( 'wtb_booking_args', $args );

		// Add an action so addons can hook in before the post type is registered
		do_action( 'wtb_booking_pre_register' );

		// Register the post type
		register_post_type( WTB_BOOKING_POST_TYPE, $args );

		// Add an action so addons can hook in after the post type is registered
		do_action( 'wtb_booking_post_register' );
	}
	...
	public function set_booking_statuses() {
		...
		$this->booking_statuses['pending'] = array(
			'label'						=> _x( 'Pending', 'Booking status when it is pending review', 'wp-table-bookings' ),
		...
		$this->booking_statuses['confirmed'] = array (
			'label'                     => _x( 'Confirmed', 'Booking status for a confirmed booking', 'wp-table-bookings' ),
			...
			'label_count'               => _n_noop( 'Confirmed <span class="count">(%s)</span>', 'Confirmed <span class="count">(%s)</span>', 'wp-table-bookings' ),
		);

		$this->booking_statuses['closed'] = array(
			'label'                     => _x( 'Closed', 'Booking status for a closed booking', 'wp-table-bookings' ),
			'default'					=> false, // Whether or not this status is part of WP Core
			'user_selectable'			=> true, // Whether or not a user can set a booking to this status
			'public'                    => false,
			'exclude_from_search'       => true,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			'label_count'               => _n_noop( 'Closed <span class="count">(%s)</span>', 'Closed <span class="count">(%s)</span>', 'wp-table-bookings' )
		);

		// Let addons hook in to add/edit/remove post statuses
		$this->booking_statuses = apply_filters( 'wtb_post_statuses_args', $this->booking_statuses );
